// find ancestors of node without recusion
// code question: 
...
public class Main
{
    public static void main(String[] er)
    {
        ...
        AncestorWithoutRecursion po = new AncestorWithoutRecursion();
        po.print(bt.getHead(), 10);
    }
}

class AncestorWithoutRecursion
{
    private Stack<Node> stack;
    public void print(Node head, Integer target)
    {
        stack = new Stack<>();
        stack.push(head);
    ...
    private void popTillNew()
    {
        ...
        while(stack.size() > 0)
        {
            if (stack.peek().getLeft() != null && stack.peek().getLeft().getValue() == popped.getValue())
            {
            ...
            else if (stack.peek().getRight().getValue() == popped.getValue())
            {
                popped = stack.pop();
            }
        }
